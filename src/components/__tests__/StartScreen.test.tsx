...
describe('StartScreen', () => {
  ...
  it('shows game rules', () => {
    ...
    expect(screen.getByText(/Teams take turns guessing words/)).toBeInTheDocument();
    expect(screen.getByText(/Correct answers earn 10 points/)).toBeInTheDocument();
  });
  ...
  it('calls onStartGame with correct settings when start button is clicked', async () => {
    ...
    expect(mockOnStartGame).toHaveBeenCalledWith({
      ...
      numberOfWords: 12,
      teams: [
        { id: expect.any(String), name: 'Team Alpha', score: 0, color: expect.any(String) },
        { id: expect.any(String), name: 'Team Beta', score: 0, color: expect.any(String) }
      ],
      answerMode: 'voice'
    });
  });
  ...
  it('shows team count limits', () => {
    ...
    expect(screen.getByText('0/20 teams • Minimum 2 teams required')).toBeInTheDocument();
  });
});
